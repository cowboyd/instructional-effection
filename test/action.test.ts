...
describe("action", () => {
  it("can resolve", async () => {
    let didClear = false;
    let task = run(() => action<number>(function*(resolve) {
  ...
  it("can reject", async () => {
    ...
    let task = run(() => action<number>(function*(_, reject) {
  ...
  it("can resolve without ever suspending", async () => {
    let result = await run(() => action<string>(function*(resolve) {
      resolve("hello");
    }));
  ...
  it("can reject without ever suspending", async () => {
    let error = new Error('boom');
    let task = run(() => action(function*(_, reject) {
      reject(error);
    }));
    await expect(task).rejects.toEqual(error);
  });
  ...
  it("can resolve before it suspends", async () => {
    expect(await run(() => action<string>(function*(resolve) {
      resolve("hello");
      yield* suspend();
    }))).toEqual("hello");
  });
  ...
  it("fails if the shutdown fails", async () => {
    let error = new Error('boom');
    let task = run(() => action(function*(resolve) {
