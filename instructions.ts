...
export function action<T>(operation: (resolve: Resolve<T>, reject: Reject) =>  Operation<void>): Operation<T> {
  return {
    *[Symbol.iterator]() {
      return yield function Action(frame) {
        return shift<Result<T>>(function*(k) {
          let result: Result<T> | undefined = void 0;
          let $resolve: Resolve<T> = (value) => { result = { type: "resolved", value } };
          let $reject: Reject = (error) => { result = { type: "rejected",  error }};
...
export function spawn<T>(operation: () => Operation<T>): Operation<Task<T>> {
  return {
    *[Symbol.iterator]() {
      return yield frame => shift<Result<Task<T>>>(function*(k) {
        let child = frame.createChild();
        ...
        } else if (exit.reason === "completed" && exit.result.type === "rejected") {
...
export function resource<T>(
  ...
  return {
    *[Symbol.iterator]() {
      return yield (frame) => shift<Result<T>>(function*(k) {
        let child = frame.createChild();
        let provide = (value: T): Operation<void> => {
          return {
            *[Symbol.iterator]() {
              return yield () => shift<Result<void>>(function*() {
        ...
        if (done.exit.reason === "completed") {
          if (done.exit.result.type === "rejected") {
            ...
            k({ type: "rejected", error: new Error(`resource exited without ever providing anything`) });
          }
        }
      });
    }
...
export function getframe(): Operation<Frame> {
  return {
    *[Symbol.iterator]() {
      return yield (frame) => shift<Result<Frame>>(function*(k) {
